...
export function QuotationSystem({ isDashboard, onClose }: { isDashboard?: boolean, onClose?: () => void }) {
  ...
  // ============================================================
  /**
   * Dispatches Step 1 customer details (Full Name, Phone, Email)
   * to the Zoho Flow Webhook and Make.com Webhook asynchronously.
   * Fail-silent try/catch ensures user interface remains completely unaffected on network errors.
   */
  const dispatchStep1Webhook = async (name: string, phone: string, email: string, method: string): Promise<void> => {
    const webhookUrl = import.meta.env.VITE_FLOW_WEBHOOK_CONTACT_CAPTURED || 
                       'https://hook.eu1.make.com/nsxcdre5idm6g2ihl2rhmjllfo3vyj6z';
    ...
    try {
      await fetch(webhookUrl, {
        ...
        body: JSON.stringify({
          ...
          email: email.trim(),
          quoteMethod: method,
          source: 'Website Quote Form Step 1',
          timestamp: new Date().toISOString(),
          referrer: typeof window !== 'undefined' ? window.location.href : '',
        }),
      });
      console.info('[Webhook] Native Step 1 lead successfully dispatched.');
    } catch (error) {
      // Fail silently without blocking UI transitions
      console.warn('[Webhook] Step 1 fetch failed silently:', error);
    }
  };
   ...
   * Dispatches webhook and triggers background tracking.
   */
  const handleStep1Continue = async (e?: React.MouseEvent | React.FormEvent): Promise<boolean> => {
    ...
    // Front-end pre-flight validation
    if (!customer.name.trim() || customer.phone.replace(/\D/g, '').length < 10 || !customer.email.includes('@')) {
      setZohoFormError('Please fill out all contact fields correctly.');
      return false;
    }
    ...
    try {
      // 1. Trigger the fail-silent direct Zoho Flow/Make.com webhook dispatch
      dispatchStep1Webhook(customer.name, customer.phone, customer.email, quoteMethod);

      // 2. Trigger background snapshot capture
      automateContactCaptured(buildQuoteSnapshot());
      
      // 3. Move cleanly to Step 2
      setStep(2);
      return true;
    } catch (err) {
      console.warn('[Quote] Step 1 transition failed:', err);
      // Fallback: still let the user proceed so they aren't frozen
      setStep(2);
      return true;
    } finally {
